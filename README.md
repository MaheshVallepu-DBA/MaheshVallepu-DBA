...
| 🔁 GoldenGate | 🏠 Oracle RAC | 🛡️ Data Guard | 🗃️ RMAN | ⚡ Tuning |
|:---:|:---:|:---:|:---:|:---:|
| Classic & Microservices | 4-Node Production | Physical Standby | Hot/Cold/Incremental | AWR Reports |
| Uni & Bi-Directional | SRVCTL & CRSCTL | Snapshot Standby | PITR Recovery | ASH Analysis |
| Extract/Replicat/Pump | OPatchAuto Rolling | DGMGRL Automation | RMAN Duplicate | ADDM Reports |
| Zero-Downtime Migration | OCR/Voting Disk | Near-Zero RPO/RTO | Data Pump EXPDP | Explain Plan |
| GGSCI Troubleshooting | TAF & FCF | Switchover/Failover | Flashback DB/Table | SQL Tuning Advisor |
...
declare -A DBA_SCRIPTS=(
  ...
  ["tablespace_monitor.sh"]="Tablespace % usage alerting — cron-ready"
  ["asm_diskgroup_check.sh"]="ASM disk group space & I/O health monitor"
  ["awr_report_gen.sh"]="Automated AWR snapshot generation & archival"
  ["dataguard_lag_alert.sh"]="ADG apply/transport lag monitoring"
  ["rac_healthcheck.sh"]="4-node RAC cluster full health dashboard"
  ["goldengate_monitor.sh"]="GoldenGate Extract/Replicat status checker"
  ["alert_log_parser.sh"]="ORA- error detection & notification engine"
)
...
- 💾 Performed **Non-ASM to ASM migrations** via RMAN — improved I/O throughput significantly
- 🛡️ Managed **OCR, Voting Disk, OLR, GPNP Profile** recovery for cluster integrity
- 📜 Automated RMAN backups, tablespace alerts, health checks via **Bash/KSH cron scripts**
...
- 🎯 Resolved P1/P2 incidents via **ServiceNow ITSM** — maintained **100% SLA adherence**
...
DECLARE
  v_projects SYS.ODCIVARCHAR2LIST := SYS.ODCIVARCHAR2LIST(
    ...
    '🤖 DBA Automation Framework — unified health-check & alerting scripts',
    '🏗️  Oracle 19c Multitenant — CDB/PDB clone, plug/unplug, Resource Mgr',
    '☁️  OCI DB Systems — Bare Metal, VM DB, cloud backup strategies'
  );
BEGIN
  FOR i IN 1..v_projects.COUNT LOOP
    DBMS_OUTPUT.PUT_LINE('  [IN PROGRESS] ' || v_projects(i));
  END LOOP;
  DBMS_OUTPUT.PUT_LINE(CHR(10) || '  STATUS : ACTIVE ⚡ | AVAILABILITY : IMMEDIATE ✅');
END;
/
```
...
|:---|:---:|:---|
| ☁️ **OCI Advanced** | `████████░░` 80% | Exadata Cloud, Autonomous DB, OCI GoldenGate |
| 🏗️ **Oracle Multitenant 19c** | `██████░░░░` 60% | CDB/PDB Admin, Application Containers |
| 📊 **Oracle Performance Mgmt** | `████████░░` 80% | Real-Time ADDM, SQL Plan Mgmt, SPM Baselines |
| 🔐 **Oracle Security** | `██████░░░░` 60% | TDE, VPD, Oracle Audit Vault, Database Vault |
| 🐍 **Python for DBA** | `████░░░░░░` 40% | cx_Oracle, python-oracledb automation |
| 🐳 **Docker / Containers** | `████░░░░░░` 40% | Oracle DB on Docker for dev/test environments |
...
```
Oracle DBA • Oracle Database Administrator • Senior DBA • Production DBA • BFSI DBA
Oracle 19c • Oracle 12c • Oracle 11g • Oracle RAC • Real Application Clusters
Oracle GoldenGate • GoldenGate Classic • GoldenGate Microservices • Real-Time Replication
Oracle Active Data Guard • Physical Standby • Snapshot Standby • DGMGRL • DR Specialist
RMAN • Backup & Recovery • PITR • Point-In-Time Recovery • Database Cloning
ASM • Automatic Storage Management • ASMCMD • Disk Group Management
OCI • Oracle Cloud Infrastructure • Cloud DBA • OCI Database Systems
SQL Performance Tuning • AWR • ASH • ADDM • Explain Plan • TKProf • SQL Tuning Advisor
OPatch • OPatchAuto • Rolling Patch • CPU Patch • PSU • Release Update (RU)
Data Pump • EXPDP • IMPDP • Zero-Downtime Migration • Database Migration
24x7 Production Support • L1 L2 DBA • ServiceNow • ITIL • Incident Management
Linux • RHEL • OEL • Shell Scripting • Bash • KSH • Cron Automation
SRVCTL • CRSCTL • TFACTL • ADRCI • GGSCI • DBCA • DBUA • DGMGRL • ORADEBUG
...
**Senior Oracle DBA &nbsp;|&nbsp; Lead DBA &nbsp;|&nbsp; Database Architect &nbsp;|&nbsp; OCI Database Engineer**
